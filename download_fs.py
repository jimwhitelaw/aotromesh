...
if env["PIOPLATFORM"] == "espressif32":
    ...
    env.Replace(
        MKSPIFFSTOOL=platform.get_package_dir("tool-mkspiffs")
        + "/mkspiffs_espressif32_arduino"
    )
...
def unpack_fs(fs_info: FSInfo, downloaded_file: str):
    ...
    if not os.path.exists(downloaded_file):
        print(
            f"ERROR: {downloaded_file} with filesystem not found, maybe download failed due to upload_speed setting being too high."
        )
        assert 0
    try:
